/*
Package confkeys defines all the configuration key alias that configure all the bundled features of bricks.
Bricks expects that a `config.yaml` or similar will be loaded as Configuration and exposed via `interfaces/cfg/Config` interface.
Bricks will access embedded keys using a `.` (dot) notation.
